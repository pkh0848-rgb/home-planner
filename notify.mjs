...
/* 집마다 오늘 할 일을 모아 그 집 구독자에게만 보낸다.
   구독 정보(pushSubs)에 어느 집인지 적혀 있어서, 집이 늘어도 그대로 동작한다. */
const DB='https://homelog-8b94f-default-rtdb.firebaseio.com';
const SITE='https://pkh0848-rgb.github.io/home-planner/';
...
/* 구독을 집별로 묶는다. house가 없는 옛 구독은 예전 단일 집으로 본다. */
const subs=await g('pushSubs')||{};
const byHouse={};
for(const key in subs){
  const e=subs[key]; if(!e) continue;
  const hid=e.house||'h_kh_home';
  (byHouse[hid]=byHouse[hid]||[]).push({key, sub:e.sub?e.sub:e});
}

let totalSent=0, totalRm=0, totalFail=0;
for(const hid in byHouse){
  const R='houses/'+hid;
  const routines=await g(R+'/routines')||{};
  const doneT=await g(R+'/done/'+today)||{};
  const exT=await g(R+'/extras/'+today)||{};
  ...
  if(wIds.length){
    const dw={};
    for(const d of week) dw[d]=await g(R+'/done/'+d)||{};
    for(const id of wIds){let done=false;for(const d of week)if(dw[d][id]){done=true;break;}if(!done)wRem++;}
  }

  const title=items.length?'🏠 오늘 할 일 '+items.length+'개':'🏠 오늘 할 일';
  ...
  for(const {key,sub} of byHouse[hid]){
    if(!sub||!sub.endpoint)continue;
    try{await wp.sendNotification(sub,payload);sent++;}
    catch(err){
      if(err.statusCode===404||err.statusCode===410){await fetch(DB+'/pushSubs/'+key+'.json',{method:'DELETE'});rm++;}
      else{fail++;console.error('push',err.statusCode);}
    }
  }
  totalSent+=sent; totalRm+=rm; totalFail+=fail;
  console.log('house',hid,'items',items.length,'wRem',wRem,'subs',byHouse[hid].length,'sent',sent,'rm',rm,'fail',fail);
}
console.log('today',today,'houses',Object.keys(byHouse).length,'sent',totalSent,'rm',totalRm,'fail',totalFail);
